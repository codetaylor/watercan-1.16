...
import com.codetaylor.mc.watercan.client.WatercanParticleSpawner;
...
public abstract class WatercanBaseItem
    extends Item {
  ...
  public WatercanBaseItem(Properties properties, String name) {
  ...
  private ActionResult<ItemStack> checkRefill(
    ...
    if (rayTraceResult != null
      ...
      // can't narrow variable type here until we've checked the result type
  ...
  private void waterBlock(
    ...
    if (this.canSpawnFlowers()
      ...
      if (world.isAirBlock(up)
        ...
        List<ConfiguredFeature<?, ?>> list = world.getBiome(up).getGenerationSettings().getFlowerFeatures();

        if (!list.isEmpty()) {
          ConfiguredFeature configuredFeature = list.get(0);
          FlowersFeature feature = (FlowersFeature) configuredFeature.feature;
          BlockState randomFlower = feature.getFlowerToPlace(RANDOM, up, configuredFeature.config);
    ...
    if (blockUpdateDelay > 0
        && block.ticksRandomly(blockState)
        && RANDOM.nextInt(40) > blockUpdateDelay) {
      blockState.randomTick((ServerWorld) world, pos, RANDOM);
    }
  }
  ...
  public static class DispenserBehavior
    ...
    public ItemStack dispense(@Nonnull IBlockSource source, @Nonnull ItemStack stack) {
      ...
      if (item instanceof WatercanBaseItem
          && ((WatercanBaseItem) item).isDispensable()) {
        ...
        if (!world.isRemote
          ...
          BlockPos offset = pos.offset(facing, ((WatercanBaseItem) item).getRange() + 1);
          ItemStack copy = stack.copy();
          ((WatercanBaseItem) item).activate(stack, world, null, offset);

          // client packet
          SCPacketDispenseWatercan packet = new SCPacketDispenseWatercan(copy, offset);
          WatercanMod.getProxy().getPacketService().sendToAll(packet);
        }
      }
